...
public class NguoiDungController {
	...
	public String viewLichKhamBenh(
	        @RequestParam(required = false) String chuyenKhoaId,
	        @RequestParam(required = false) String ngayThangNam,
	        Model model) {
	    ...
	    List<LichKhamBenh> lichKhamBenhPage = lichKhamBenhService.findLichKhamBenhByChuyenKhoaAndNgay(
	            chuyenKhoaId, ngay);
	...
	@GetMapping("/findbs")
	public String findBacSi(
	    @RequestParam(name = "section", required = false, defaultValue = "all") String section,
	    @RequestParam(name = "page", required = false, defaultValue = "0") int page,
	    @RequestParam(name = "size", required = false, defaultValue = "5") int size,
	    @RequestParam(name = "doctorName", required = false) String doctorName, // Thêm tham số tìm kiếm theo tên bác sĩ
	    Model model) {
	    ...
	    if (doctorName != null && !doctorName.isEmpty()) {
	        bacSiPage = bacSiService.findByNameAndChuyenKhoa(doctorName, section, pageable);  // Tìm kiếm theo tên và chuyên khoa
	...
	public String viewChiTietBacSi(@PathVariable("bacSiId") String bacSiId, Model model) {
	 ...
	 public String proceedToStep2(@RequestParam("doctor") String doctorId, @RequestParam("service") String serviceId, Model model) {
	 ...
	 public LocalTime calculateStartTime(String selectedTime) {
		...
		public ResponseEntity<Map<String, String>> confirmBooking(@RequestBody Map<String, String> requestData) {
		    ...
		    DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
		    LocalDate selectedDates = LocalDate.parse(selectedDate, formatter);
		    ...
		    SlotThoiGian check = slotThoiGianService.findExist(calculateStartTime(selectedTime),benhnhan.getBenhNhanId(),lichKhamBenh.getMaLichKhamBenh());
		   System.out.println(check.getTrangThai()+"hehe");
